 ...
 * @package    Twitter
 */
class TwitterText_Validation extends TwitterText_Regex {
  ...
  public function validateUsername() {
    $length = mb_strlen($this->tweet);
    if (!$this->tweet || !$length) return false;
    $extracted = TwitterText_Extractor::create($this->tweet)->extractMentionedUsernames();
  ...
  public function validateHashtag() {
    $length = mb_strlen($this->tweet);
    if (!$this->tweet || !$length) return false;
    $extracted = TwitterText_Extractor::create($this->tweet)->extractHashtags();
    return count($extracted) === 1 && $extracted[0] === substr($this->tweet, 1);
  }
  ...
  public function getLength() {
    $length = mb_strlen($this->tweet);
    $urls_with_indices = TwitterText_Extractor::create($this->tweet)->extractURLsWithIndices();
